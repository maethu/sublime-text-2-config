...
def check_and_mark(view):
    ...
    base_path = _find_buildout(view.file_name())
    print 10*"*" + " RUN CHECKER " + 10*"*"
    for checker, args in CHECKERS:
        checker_messages = []
        try:
            p = Popen([base_path +'/'+checker, view.file_name()] + args, stdout=PIPE,
                stderr=PIPE)
...
def _find_buildout(file_dir):
    def _traverse_upward(look_for, start_at="."):
        ...
        while True:
            ...
            if new_p == p:
                return '/Users/maethu/python2.4/python-2.6'
            p = new_p

    return _traverse_upward(
        "bootstrap.py", start_at=os.path.split(file_dir)[0])
